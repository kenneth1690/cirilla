...
abstract class ProductsStoreBase with Store {
  ...
  @action
  Future<List<Product>> getProducts({CancelToken? token, Map<String, dynamic>? customQuery}) async {
    if (pending) {
    ...
    Map<String, dynamic> query = {
      "page": _nextPage,
      "category":
          _filterStore?.categories.isNotEmpty == true ? _filterStore!.categories.map((c) => c.id).join(",") : '',
